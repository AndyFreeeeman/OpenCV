...
def drawBbox(img,bbox):#,T0):
    ...
    for box in bbox:
        ...
        cv2.rectangle(img, (x0, y0), (x1, y1), (0, 255, 0), 7)
...
def old_drawBbox(img,bbox):#,T0):
    #T = time.time()-T0
    #cv2.putText(img, "fps: "+str(1/T), (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 1, cv2.LINE_AA)
    for box in bbox:
        x0,y0,x1,y1 = box[0:4].astype(int)
        cv2.rectangle(img, (x0, y0), (x1, y1), (255, 0, 255), 7)
        cv2.putText(img, str(int(box[-1])), (x0, y0+10), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 1, cv2.LINE_AA)
        #cv2.putText(img, str((x0,y0)), (x0, y0), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 1, cv2.LINE_AA)
    return img 
...
class Detection():
    def __init__(self,model='YOLOv5',weight='./weights/yolov5s.pt',im0_shape=(720,960)):
        if(model=='YOLOv5'):
            self.Model = attempt_load(weight, map_location=torch.device('cuda:1'))
        self.Model.eval()
    ...
    def inference(self,img):
        ...
        pred = non_max_suppression(pred, 0.77, 0.6)
        if(pred[0] != None):
            pred = pred[0]
    ...
    def batchInference(self,imgList):
        ...
        preds = non_max_suppression(preds, 0.77, 0.6)
        result = []
        for pred in preds:
            if(pred != None):
                pred[:, [0, 2]] -= self.pad[0]  # x padding
